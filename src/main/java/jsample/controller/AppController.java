...
public class AppController {
    ...
    @ResponseBody
    public ResponseEntity<Mono<?>> getAppById(@PathVariable String id) {
        return new ResponseEntity<Mono<?>>(service.getAppById(id), HttpStatus.OK);
    }

    @PostMapping(value = {AppConstants.URI.ADD_APP})
    @ResponseBody
    public ResponseEntity<Mono<?>> addApp(@RequestBody Application app) {
        return new ResponseEntity<Mono<?>>(service.addApp(app), HttpStatus.CREATED);
    }
